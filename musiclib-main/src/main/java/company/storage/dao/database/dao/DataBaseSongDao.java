package company.storage.dao.database.dao;

import company.entityclass.Song;
import company.exception.DBException;
...
public class DataBaseSongDao implements SongDao {
    ...
    public DataBaseSongDao(MyConnection connection) {
        try {
            ...
            renameSong = con.prepareStatement("UPDATE song  SET songname = ? , lengthmin = ?  WHERE song_id = ?");
    ...
    public void updateSong(Song song) throws SQLException {
        try {
            renameSong.setString(1, song.getName());
            renameSong.setLong(2, song.getLength());
            renameSong.setString(3,song.getId().toString());
            renameSong.execute();
            con.commit();
        } catch (SQLException throwable) {
            con.rollback();
